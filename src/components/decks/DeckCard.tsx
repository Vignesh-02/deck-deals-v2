...
export default function DeckCard({ deck }: { deck: IDeck }) {
  ...
  return (
    <Link href={`/decks/${deck._id}`} className="group block">
      <div className="relative overflow-hidden rounded-lg border border-[#2a2a2a] bg-surface transition-all duration-300 group-hover:border-brand-gold/30 group-hover:shadow-xl group-hover:shadow-brand-gold/5">
        {/* Image */}
        <div className="relative aspect-[3/4] overflow-hidden bg-surface-darker">
          {coverImage ? (
            <img
              src={coverImage}
              alt={deck.name}
          ...
          {/* Image count badge */}
          {imageCount > 1 && (
            <div className="absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-black/60 backdrop-blur-sm text-cream text-[10px] font-medium">
        ...
        {/* Info */}
        <div className="p-3">
          <h3 className="font-display text-base tracking-[0.04em] uppercase text-cream truncate group-hover:text-brand-gold transition-colors">
            {deck.name}
          </h3>
          <div className="mt-1.5 flex items-center justify-between gap-2">
            ...
            <span className="font-display text-base text-brand-gold">
              &#8377;{deck.price}
            </span>
          </div>
        </div>
